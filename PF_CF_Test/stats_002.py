...
# OCV, DRT
dummy_snd_inf = in_f_df[["OCV-B", "Max Gamma"]]
...
snd_df = {}
for f in f_df_list:
    dummy = pf_raw_df[f][["Zre (mohm)", "Zim (mohm)", "Mag (mohm)", "Phase"]]
...
for i, ft in enumerate(["OCV-B", "Max Gamma"]):
...
sdf_draw_df_0_numeric = sdf_draw_df_0.select_dtypes(include=[np.number]).drop(["P/F"], axis=1)
sdf_draw_df_1_numeric = sdf_draw_df_1.select_dtypes(include=[np.number]).drop(["P/F"], axis=1)
...
for f in f_df_list:
    sdf_draw_df_0 = snd_df[f][snd_df[f]["P/F"] == 0]
...
intersection_dict = {ft: [] for ft in ["OCV-B", "Max Gamma"]}
left_dict = {ft: [] for ft in ["OCV-B", "Max Gamma"]}
right_dict = {ft: [] for ft in ["OCV-B", "Max Gamma"]}

# KDE 함수를 저장할 딕셔너리 생성
kde_functions_0 = {ft: [] for ft in ["OCV-B", "Max Gamma"]}
kde_functions_1 = {ft: [] for ft in ["OCV-B", "Max Gamma"]}
...
intersection_ind_df = pd.DataFrame.from_dict(intersection_dict, orient='index').transpose()
...
# intersection_ind_df에 왼쪽(left)과 오른쪽(right) 적분 값을 각각 추가
intersection_ind_df = pd.concat([intersection_ind_df, left_df.add_prefix("Left_"), right_df.add_prefix("Right_")], axis=1)
intersection_ind_df*np.array(1.0e6)
...
# KDE 함수를 저장할 딕셔너리 생성
kde_functions_0 = {ft: [] for ft in ['Zre (mohm)', 'Zim (mohm)', 'Mag (mohm)', 'Phase']}
kde_functions_1 = {ft: [] for ft in ['Zre (mohm)', 'Zim (mohm)', 'Mag (mohm)', 'Phase']}
kde_dataframes_0 = {}
kde_dataframes_1 = {}
...
# 기존의 intersection_df를 생성하는 코드
intersection_eis_df = pd.DataFrame.from_dict(intersection_dict, orient='index').transpose()
intersection_eis_df = pd.concat([intersection_eis_df, pd.DataFrame([1, 10, 100, 1000], columns=["Frequency"])], axis=1)

# left_dict와 right_dict를 데이터프레임으로 변환
left_df = pd.DataFrame.from_dict(left_dict, orient='index').transpose()
right_df = pd.DataFrame.from_dict(right_dict, orient='index').transpose()

# intersection_eis_df에 왼쪽(left)과 오른쪽(right) 적분 값을 각각 추가
intersection_eis_df = pd.concat([intersection_eis_df, left_df.add_prefix("Left_"), right_df.add_prefix("Right_")], axis=1)
# intersection_eis_df.to_excel("C:/Users/jeongbs1/Downloads/PUR_Calculation.xlsx")
...
intersection_eis_df[inter_df_col]
...
for i in range(num_plots):
    ...
    for l, ft in enumerate(inter_df_col[start_idx + 1:end_idx + 1]):
        axs[row][col].plot(list(intersection_eis_df.index[:]), intersection_eis_df[ft][:] * np.array(1.0e6), "o-",label=label[l])

    axs[row][col].set_title(inter_df_col[end_idx-2])
    axs[row][col].set_xlabel("Frequency(Hz)")
    axs[row][col].set_ylabel("PUR(ppm)")
    axs[row][col].set_xticks(ticks=[list(intersection_eis_df.index)[i] for i in [0, 1, 2, 3]], labels=[1, 10, 100, 1000])
...
for i in range(num_plots):
    ...
    for l, ft in enumerate(inter_df_col[start_idx + 1:end_idx + 1]):
        axs[row][col].plot(
            list(intersection_eis_df.index[:]),
            intersection_eis_df[ft][:] * np.array(1.0e6),
            "o-",
            label=label[l],)
    axs[row][col].set_title(inter_df_col[end_idx - 2])
    axs[row][col].set_xlabel("Frequency(Hz)")
...
intersection_ind_df.iloc[:, :2]*np.array(1.0e6) - intersection_ind_pdf_df.iloc[:, :2]*np.array(1.0e6)
...
for f in f_df_list:
    ...
    # 각 피처에 대해 PDF 생성 및 적분 계산
    for ft in ["Zre (mohm)", "Zim (mohm)", "Mag (mohm)", "Phase"]:
        dummy_draw_0 = sdf_draw_df_0[ft]
        dummy_draw_1 = sdf_draw_df_1[ft]
...
compare_pur_kde = intersection_eis_df[['Zre (mohm)', 'Zim (mohm)', 'Mag (mohm)', 'Phase']]
compare_pur_pdf = intersection_eis_pdf_df[['Zre (mohm)', 'Zim (mohm)', 'Mag (mohm)', 'Phase']]
fig, axs = plt.subplots(ncols=2, nrows=2, figsize=(12, 8))
fig.suptitle("PUR based on EIS Features")
features_per_plot = 1
num_plots = len(inter_df_col[1:]) // features_per_plot
for i, ft in enumerate(['Zre (mohm)', 'Zim (mohm)', 'Mag (mohm)', 'Phase']):
    ...
    axs[row][col].plot(list(intersection_eis_df.index[:]),compare_pur_pdf[ft][:] * np.array(1.0e6),"o-",label="PDF")
    axs[row][col].set_title(ft)
    axs[row][col].set_xlabel("Frequency(Hz)")
    axs[row][col].set_ylabel("PUR(ppm)")
    axs[row][col].set_xticks(ticks=[list(intersection_eis_pdf_df.index)[i] for i in [0, 1, 2, 3]], labels=[1, 10, 100, 1000])
